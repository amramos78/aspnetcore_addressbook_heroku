...
namespace AddressBook.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel resgisterViewModel)
        {
            ...
            {
                var user = new IdentityUser{ UserName = resgisterViewModel.EmailAddress, Email = resgisterViewModel.EmailAddress};
                var result = await _userManager.CreateAsync(user, resgisterViewModel.Password);
                if(result.Succeeded)
                {
